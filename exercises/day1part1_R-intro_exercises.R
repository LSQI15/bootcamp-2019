...
# Check your working directory. It should be the root folder where you downloaded the boot camp materials. 
#If that's not the case, set your working directory accordingly.


# Read gapminder data with read.csv()
gapminder <- read.csv("data/gapminder5.csv", stringsAsFactors=FALSE) ##avoid converting to factors ##default separtor is ','
...
###ASK
#########
gapminder <- read_csv("data/gapminder5.csv") ##This is from the readr package and you can learn about the data type
...
table(gapminder07$continent) %>% sort()
...
gapminder07[order(gapminder07$pop,decreasing=T),] %>% head(5)
...
aggregate(lifeExp ~ continent,gapminder07, mean) 
aggregate(gapminder07$lifeExp ~ gapminder07$continent, FUN = mean) ##must indicate FUN


aggregate(gapminder07,list(gapminder07$continent),mean) #nahnahnah
...
write.csv(gapminder07,file = '/Users/siqili/Desktop/bootcamp-2019/data/gapminder07.csv',row.names = FALSE) #You don't need to add row.names = FALSE if you are using write_csv()
...
hist(gapminder07$lifeExp,main = 'this is a histogram',xlab = 'life exp')


# Bonus: Change the `breaks = ` argument from its default setting and see what happens.
hist(gapminder07$lifeExp,main = 'this is a histogram',xlab = 'life exp',breaks = 20)
...
plot(x=gapminder07$gdpPercap,y=gapminder07$lifeExp, main = 'This is a scatterplot', 
