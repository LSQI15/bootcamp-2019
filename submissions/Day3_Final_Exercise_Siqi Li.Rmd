...
```{r}
str(nys_acs) %>% pander()
summary(nys_acs) %>% pander()
str(nys_schools) %>% pander()
summary(nys_schools) %>% pander()
```
...
#task 2
#this one is based on the median household income:
#Levels: (3.37e+04,5.65e+04] (5.65e+04,7.93e+04] (7.93e+04,1.02e+05]
nys_acs$group_range = cut(nys_acs$median_household_income, 3) 
# show the group number
nys_acs$group_index = as.numeric(cut(nys_acs$median_household_income, 3)) 
# modify into high median or low
nys_acs$group[nys_acs$group_index==1]='low'
nys_acs$group[nys_acs$group_index==2]='medium'
nys_acs$group[nys_acs$group_index==3]='high'

#task 3
nys_schools %>% select(year,mean_ela_score,mean_math_score) %>% group_by(year) %>% summarise()
